...
Trend <- function(Date, Place, ExtData, Type){
  if (Date!='2020-03-07'){
    ...
    if (Type == "NORMAL"){
      if (Place=='UK'){
        data <- data.frame('date' = format(as.Date(names(total)), format="%Y-%m-%d"),
                           'total'=ExtData$NumPositive[10:which(rownames(ExtData)==as.character(Date))],
    ...
    if (Type == 'LOG'){
      if (Place=='UK'){
        data <- data.frame('date' = format(as.Date(names(total)), format="%Y-%m-%d"),
                           'total'=log10(ExtData$NumPositive[10:which(rownames(ExtData)==as.character(Date))]),
    ...
    if (Type == 'DailyCases'){
      if (Place=='UK'){
        ...
        fig <- plot_ly(data = data, x = ~Date, y = ~DiffNumPositive, 
                       ...
                       #width = 430,
                       height = 300)%>%
          add_trace(data = data, x =~Date, y=~SevenDayAvg, name = '7-Day Average', type = 'scatter', mode = 'lines', line = list(color = 'rgb(255,165,0)'))
      }
    ...
    if (Type == 'DailyDeaths'){
      ...
      if (Place == 'UK'){
        fig <- plot_ly(data = data, x = ~Date, y = ~DiffDeaths, 
                       ...
                       #width = 430,
                       height = 300)%>%
          add_trace(data = data, x =~Date, y=~SevenDayAvgDeaths, name = '7-Day Average', type = 'scatter', mode = 'lines', line = list(color = 'rgb(255,165,0)'))
      }
...
# Tables ------------------------------------------------------------------
  ...
  output$dataset5 <- DT::renderDataTable({
    DT::datatable(df5[order(df5$Date, decreasing = TRUE),],
                  rownames = FALSE,
                  extensions = 'Buttons',
                  ...
                  class = "display"
    )%>%formatStyle(colnames(df5), backgroundColor = 'black')
  })
